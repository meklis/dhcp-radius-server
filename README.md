...
конфігу вище) і застосовувати одиничні `add`/`update`/`delete` по конкретному `id` запису,
не чіпаючи решту binds і не роблячи повний reload. Повідомлення в каналі - JSON:
...
`db_type` повинен збігатися з одним із ключів `script.database.binds` (наприклад `"clients"`/
`"smart"`, або одне ім'я `"binds"`, якщо джерело одне) - подія для незнайомого `db_type`
відхиляється з помилкою в лозі, а не створює джерело "на льоту". `object.ip` - той самий
формат, що й у HTTP-джерелі (IPv4 одним числом, див. нижче). `object.device_mac`/`object.port`
опціональні - їх відсутність означає запис без прив'язки до пристрою/порту (як у "smart").
`action: "add"` і `"update"` обробляються однаково (upsert по `object.id`): якщо запис із
таким `id` вже є - замінюється цілком, якщо немає - створюється. Це не заміна повного reload,
а доповнення для меншої затримки - наступний плановий reload все одно повністю перезапитає
джерело і є єдиним, хто насправді прибирає застарілі записи "за замовчуванням" (якщо зовнішня
система не надіслала відповідний `delete`).

Повний контракт цього JSON-формату (для команди, яка реалізує відправку таких повідомлень
у зовнішній системі) - [doc/REDIS_BIND_EVENTS.md](doc/REDIS_BIND_EVENTS.md).

**Гонка з reload.** Поки триває сам reload (від початку HTTP-запиту до джерела і до
фактичної публікації нового знімку - для великих binds це може бути секунди), точкові
live-події не застосовуються одразу до знімку, що от-от буде замінений цілком (інакше
вони губилися б непомітно) - вони буферизуються і накатуються на вже свіжий знімок одразу
після reload, у тому ж порядку, в якому прийшли. Тобто подія, що прийшла під час reload,
з невеликою затримкою (до кінця поточного reload), але гарантовано застосується - не
губиться і не лягає "не на ту" версію даних.
...
### Як запустити       
1. Docker - зібрати образ самостійно з вихідного коду (опис в ./install/docker),
   або запустити готовий образ з ghcr.io через docker-compose без клонування
   репозиторію (опис в ./install/compose)
